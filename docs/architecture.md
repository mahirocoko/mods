...
The repository root is one installable Letta package with three manifest entries. This deliberately favors one-command private Git installation and one update source over independent npm publication.
...
The declared capability list is the union of the three entries. Runtime behavior still remains independent because each activation function checks the capabilities it needs.
...
- The bundle matches Mahiro's current machine, where these three mods are intentionally active together.
