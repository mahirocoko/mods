...
function formatRunList(runs: Run[]): string { if (!runs.length) return "No Execution Runs are stored for this agent."; return ["Mahiro Execution Runs · human-only inventory", ...runs.sort((left, right) => right.updated_at.localeCompare(left.updated_at)).map((run) => `${run.id} · ${run.stage} · rev ${run.revision} · ${run.conversation_id} · ${run.updated_at.slice(0, 10)} · ${compactRunSummary(run.summary)}`), "Use /mh-run abandon <run-id> <revision> [note] or /mh-run clear <run-id> <revision> with explicit human intent."].join("\n"); }
