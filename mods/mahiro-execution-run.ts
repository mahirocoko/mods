...
function format(run: Run): string { return [`Mahiro Execution Run · ${run.stage} · revision ${run.revision}`, `Summary: ${run.summary}`, `Targets: ${run.targets.length} · lanes: ${run.lanes.length} · open blockers: ${openBlockers(run).length}`, `Reports: ${run.lanes.reduce((count, item) => count + item.reports.length, 0)} · handoff: ${run.handoff ? "recorded" : "not recorded"}`, "Metadata is coordination only, not proof."].join("\n"); }
function output(message: string, success = true) { return { type: "output" as const, output: message, success }; }
function command(ctx: any) { const input = String(ctx?.args ?? "").trim(); try { const scope = scopeFrom(ctx); if (!input || input === "status") { const run = getRun(scope); return output(run ? format(run) : "No Execution Run exists for this scope."); } if (input === "unlock --force") return output(forceUnlock() ? "Execution Run mutation lock quarantined and removed." : "No Execution Run mutation lock exists."); if (input === "unlock") return output("Use /mh-run unlock --force only after confirming the owner is gone.", false); const clearMatch = input.match(/^clear\s+(\d+)$/); if (clearMatch) return output(clear(scope, Number(clearMatch[1])) ? "Execution Run cleared." : "No Execution Run exists for this scope."); const abandonMatch = input.match(/^abandon\s+(\d+)(?:\s+([\s\S]+))?$/); if (abandonMatch) return output(format(abandon(scope, Number(abandonMatch[1]), optionalText(abandonMatch[2], "note", MAX_LONG)))); return output("Usage: /mh-run status | clear <revision> | abandon <revision> [note] | unlock --force", false); } catch (error) { return output(error instanceof Error ? error.message : String(error), false); } }
